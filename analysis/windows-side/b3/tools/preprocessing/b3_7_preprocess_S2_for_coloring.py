...
# 平滑化窓
SMOOTH_WINDOWS = {
    "d_speed_kmh_dt": 11,     # 推進包絡は少し強めに平滑化
    "ax_g_s": 5,              # 局所アクセントなので中程度
    "pitch_deg_s": 7,         # 陰影用
    "d_pitch_deg_s_dt": 5,    # 変化点用
    "yaw_deg_s": 7,
    "d_yaw_deg_s_dt": 5,
}
...
TRANSITION_EVENT_THRESHOLD = 0.60
...
def find_section_csv(run_dir: Path, section_id: str) -> Path:
    ...
    例:
      ...
      対象ファイル = run_20260308_time_S2_timeseries.csv
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="S2時系列CSVから必要列を抽出し、補間・平滑化・規格化して色付け用制御信号を作る"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    # =====================================================
    # 推進の包絡: d_speed主体
    out["propulsion_envelope"] = out["d_speed_kmh_dt_norm"]

    # 局所アクセント: ax由来
    out["accent_strength"] = to_01_from_norm(out["ax_g_s_norm"])

    # 開放感の陰影:
    # pitchは逆相関側なので、そのまま残してもよいが、
    # 「持ち上がり感」の制御としては反転版も用意しておく
    out["lift_shadow"] = (-out["pitch_deg_s_norm"]).clip(-1.0, 1.0)

    # 変化イベント
    out["transition_event_strength"] = out["d_pitch_deg_s_dt_norm"].abs().clip(0.0, 1.0)
    out["transition_event"] = (
        out["transition_event_strength"] >= TRANSITION_EVENT_THRESHOLD
    ).astype(int)

    # 任意の補助列
    if "yaw_deg_s_norm" in out.columns:
        out["curve_hint"] = out["yaw_deg_s_norm"]
    if "d_yaw_deg_s_dt_norm" in out.columns:
