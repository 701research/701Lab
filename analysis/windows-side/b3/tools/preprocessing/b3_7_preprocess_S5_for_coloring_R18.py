...
# S5で使う必須列
# 第2メインなので yaw 主役 + ay/ax をしっかり入れる
REQUIRED_COLUMNS = [
    "t_rel_sec",
    "yaw_deg_s",
    "d_yaw_deg_s_dt",
...
def find_section_csv(run_dir: Path, section_id: str) -> Path:
    ...
    例:
      ...
      対象ファイル = run_20260322_time_S5_timeseries.csv
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="R18 S5時系列CSVから必要列を抽出し、補間・平滑化・規格化してドラム用制御信号を作る"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    # 長周期包絡:
    # yaw主体 + ay を加えて横Gのふくらみを反映
    envelope_src = (
        0.70 * out["yaw_deg_s_norm"]
        + 0.30 * out["ay_g_s_norm"]
    )
    out["drum_envelope"] = clip_pm1(envelope_src)

    # 局所アクセント:
    # |d_yaw|主体 + |d_ay| を補助に
    accent_src = 0.70 * out["d_yaw_deg_s_dt_norm"].abs()

    if "d_ay_g_s_dt_norm" in out.columns:
        accent_src = (
            0.65 * out["d_yaw_deg_s_dt_norm"].abs()
            + 0.35 * out["d_ay_g_s_dt_norm"].abs()
        )

    out["drum_accent"] = clip_01(accent_src)

    # 打撃強度:
    # ax主体 + |ay| を加えて、前後だけでなく横方向の張りも打撃感に反映
    velocity_src = (
        0.70 * to_01_from_norm(out["ax_g_s_norm"])
        + 0.30 * out["ay_g_s_norm"].abs()
    )
    out["drum_velocity"] = clip_01(velocity_src)

    # 密度:
    # d_speed主体 + |ay| を少量加えて、R18らしい塊感のある詰まり方にする
    density_src = (
        0.80 * out["d_speed_kmh_dt_norm"]
        + 0.20 * out["ay_g_s_norm"].abs()
    )
    out["drum_density"] = clip_pm1(density_src)

    # 追加打撃トリガー
    out["drum_event"] = (
        out["drum_accent"] >= DRUM_EVENT_THRESHOLD
    ).astype(int)

    # 任意補助列
    if "gz_dps_s_norm" in out.columns:
        out["drum_accent_gz"] = out["gz_dps_s_norm"].abs().clip(0.0, 1.0)

    if "roll_deg_s_norm" in out.columns:
        out["curve_roll_hint"] = out["roll_deg_s_norm"]

    if "pitch_deg_s_norm" in out.columns:
        out["melodic_shadow_hint"] = out["pitch_deg_s_norm"]
    ...
    print(f"[INFO] drum_event threshold: {DRUM_EVENT_THRESHOLD}")
