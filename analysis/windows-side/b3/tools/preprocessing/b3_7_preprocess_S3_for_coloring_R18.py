...
# S3で使う必須列
# 第一メインなので pitch を主役にしつつ、R18らしさとして ay / yaw を補助的に加える
REQUIRED_COLUMNS = [
    "t_rel_sec",
    "pitch_deg_s",
    "d_pitch_deg_s_dt",
...
def find_section_csv(run_dir: Path, section_id: str) -> Path:
    ...
    例:
      ...
      対象ファイル = run_20260322_time_S3_timeseries.csv
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="R18 S3時系列CSVから必要列を抽出し、補間・平滑化・規格化して保存する"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    # =====================================================
    # S3専用 制御信号（R18版）
    # 第一メインなので、pitch 主役を崩さない
    # =====================================================

    # 旋律アーク:
    # pitch主体 + yawを少量だけ加える
    melody_arc_src = (
        0.85 * out["pitch_deg_s_norm"]
        + 0.15 * out["yaw_deg_s_norm"]
    )
    out["melody_arc"] = clip_pm1(melody_arc_src)

    # 旋律イベント:
    # |d_pitch|主体 + |d_yaw|を少しだけ補助
    melody_event_src = (
        0.80 * out["d_pitch_deg_s_dt_norm"].abs()
        + 0.20 * out["d_yaw_deg_s_dt_norm"].abs()
    )

    if "d_ay_g_s_dt_norm" in out.columns:
        melody_event_src = (
            0.75 * out["d_pitch_deg_s_dt_norm"].abs()
            + 0.15 * out["d_yaw_deg_s_dt_norm"].abs()
            + 0.10 * out["d_ay_g_s_dt_norm"].abs()
        )

    out["melody_event_strength"] = clip_01(melody_event_src)

    # アクセント:
    # ax主体 + |ay|を補助で入れて、R18の横方向の張りを少しだけ反映
    accent_src = (
        0.75 * to_centered_01(out["ax_g_s_norm"])
        + 0.25 * out["ay_g_s_norm"].abs()
    )
    out["accent_strength"] = clip_01(accent_src)

    # フレーズ推進:
    # d_speed主体を維持しつつ、R18らしく少量だけ ax と |ay| を足す
    phrase_drive_src = (
        0.75 * out["d_speed_kmh_dt_norm"]
        + 0.15 * out["ax_g_s_norm"]
        + 0.10 * out["ay_g_s_norm"].abs()
    )
    out["phrase_drive"] = clip_pm1(phrase_drive_src)

    # melody_event の2値版
    out["melody_event"] = (
        out["melody_event_strength"] >= MELODY_EVENT_THRESHOLD
    ).astype(int)
    ...
    print(f"[INFO] melody_event threshold: {MELODY_EVENT_THRESHOLD}")
