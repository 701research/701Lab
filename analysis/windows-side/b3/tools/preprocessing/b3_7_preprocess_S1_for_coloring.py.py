...
def find_section_csv(run_dir: Path, section_id: str) -> Path:
    ...
    例:
      ...
      対象ファイル = run_20260308_time_S1_timeseries.csv
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="S1時系列CSVから必要列を抽出し、補間・平滑化・規格化して色付け用制御信号を作る"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    # 導入の陰影
    # pitchは逆相関側なので、陰影としては反転版を持っておく
    out["shadow_shape"] = (-out["pitch_deg_s_norm"]).clip(-1.0, 1.0)

    # 微小イベント
    out["intro_event_strength"] = out["d_pitch_deg_s_dt_norm"].abs().clip(0.0, 1.0)
    out["intro_event"] = (
