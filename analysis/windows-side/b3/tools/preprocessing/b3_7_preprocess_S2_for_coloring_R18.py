...
# 平滑化窓
SMOOTH_WINDOWS = {
    "d_speed_kmh_dt": 11,     # 推進包絡
    "ax_g_s": 5,              # 前後アクセント
    "ay_g_s": 5,              # 横方向の張り
    "pitch_deg_s": 7,         # 陰影
    "d_pitch_deg_s_dt": 5,    # 変化点
    "yaw_deg_s": 7,           # 向きのうねり
...
# transition_event しきい値
TRANSITION_EVENT_THRESHOLD = 0.58
...
def find_section_csv(run_dir: Path, section_id: str) -> Path:
    ...
    例:
      ...
      対象ファイル = run_20260322_time_S2_timeseries.csv
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="R18 S2時系列CSVから必要列を抽出し、補間・平滑化・規格化して色付け用制御信号を作る"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    # =====================================================
    # 推進の包絡:
    # d_speed主体 + ax + yaw + |ay| で
    # 「前進しながら車体がうねる」感じを反映
    propulsion_src = (
        0.45 * out["d_speed_kmh_dt_norm"]
        + 0.20 * out["ax_g_s_norm"]
        + 0.20 * out["yaw_deg_s_norm"]
        + 0.15 * out["ay_g_s_norm"].abs()
    )
    out["propulsion_envelope"] = clip_pm1(propulsion_src)

    # 局所アクセント:
    # ax主体 + |ay| を少し加える
    accent_src = (
        0.75 * to_01_from_norm(out["ax_g_s_norm"])
        + 0.25 * out["ay_g_s_norm"].abs()
    )
    ...
    # -pitch主体 + yaw + |ay|
    lift_src = (
        0.60 * (-out["pitch_deg_s_norm"])
        + 0.25 * out["yaw_deg_s_norm"]
        + 0.15 * out["ay_g_s_norm"].abs()
    )
    out["lift_shadow"] = clip_pm1(lift_src)

    # 変化イベント:
    # |d_pitch|主体 + |d_yaw| を強めに
    event_src = (
        0.60 * out["d_pitch_deg_s_dt_norm"].abs()
        + 0.40 * out["d_yaw_deg_s_dt_norm"].abs()
    )

    if "d_ay_g_s_dt_norm" in out.columns:
        event_src = (
            0.55 * out["d_pitch_deg_s_dt_norm"].abs()
            + 0.30 * out["d_yaw_deg_s_dt_norm"].abs()
            + 0.15 * out["d_ay_g_s_dt_norm"].abs()
        )

    out["transition_event_strength"] = clip_01(event_src)
    out["transition_event"] = (
        out["transition_event_strength"] >= TRANSITION_EVENT_THRESHOLD
    ).astype(int)
    ...
    print(f"[INFO] transition_event threshold: {TRANSITION_EVENT_THRESHOLD}")
