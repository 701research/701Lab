...
def find_section_csv(run_dir: Path, section_id: str) -> Path:
    ...
    例:
      ...
      対象ファイル = run_20260308_time_S5_timeseries.csv
    """
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="S5時系列CSVから必要列を抽出し、補間・平滑化・規格化してドラム用制御信号を作る"
    )
    parser.add_argument(
        "--base-dir",
        type=str,
    ...
    parser.add_argument(
        "--include-optional",
        action="store_true",
        help="gz/roll/pitch など任意列も含めて出力する",
    )
    ...
    # =====================================================
    # 長周期包絡: yawの位相・膨らみ
    out["drum_envelope"] = out["yaw_deg_s_norm"]

    # 局所アクセント: yaw変化の強い場所
    out["drum_accent"] = out["d_yaw_deg_s_dt_norm"].abs().clip(0.0, 1.0)

    # 打撃強度: ax由来
    out["drum_velocity"] = to_01_from_norm(out["ax_g_s_norm"])

    # 密度: 前進感
    out["drum_density"] = out["d_speed_kmh_dt_norm"]

    # 追加打撃トリガー
    out["drum_event"] = (out["drum_accent"] >= DRUM_EVENT_THRESHOLD).astype(int)
