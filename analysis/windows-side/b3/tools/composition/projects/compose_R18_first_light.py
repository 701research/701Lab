...
MIDI_OUT = (BASE_DIR / "R18_701lab_v2_17_retimed_CelloOctDown_Bass_sidegroove_S3_drum.mid").resolve()
WAV_OUT  = (BASE_DIR / "R18_701lab_v2_17_retimed_CelloOctDown_Bass_sidegroove_S3_drum.wav").resolve()

FLUIDSYNTH = Path(r"C:\fluidsynth\bin\fluidsynth.exe").resolve()
SOUNDFONT  = Path(r"C:\FluidR3_GM\FluidR3_GM.sf2").resolve()

# R18 run_id
R18_RUN_ID = "run_20260322_054454_699988"
R18_PREFIX = "run_20260322"

# S1 前処理済みCSV
S1_CONTROL_CSV = Path(
    rf"D:\701lab\work\phaseB\analysis\b3\session_timeseries\by_run\{R18_RUN_ID}\preprocessed_for_coloring\{R18_PREFIX}_time_S1_preprocessed_for_coloring.csv"
).resolve()

# S2 前処理済みCSV
S2_CONTROL_CSV = Path(
    rf"D:\701lab\work\phaseB\analysis\b3\session_timeseries\by_run\{R18_RUN_ID}\preprocessed_for_coloring\{R18_PREFIX}_time_S2_preprocessed_for_coloring.csv"
).resolve()

# S3 前処理済みCSV
S3_CONTROL_CSV = Path(
    rf"D:\701lab\work\phaseB\analysis\b3\session_timeseries\by_run\{R18_RUN_ID}\preprocessed_for_coloring\{R18_PREFIX}_time_S3_preprocessed_for_coloring.csv"
).resolve()

# S5 前処理済みCSV
S5_CONTROL_CSV = Path(
    rf"D:\701lab\work\phaseB\analysis\b3\session_timeseries\by_run\{R18_RUN_ID}\preprocessed_for_coloring\{R18_PREFIX}_time_S5_preprocessed_for_coloring.csv"
).resolve()
...
S2_EVENT_VEL_DROP = 9
S2_GRACE_RATIO = 0.17
S2_EVENT_THRESHOLD = 0.52
S2_MIN_EVENT_GAP_SEC = 0.36

# -----------------------------
# S3 色付けパラメータ
# -----------------------------
S3_PITCH_STEPS_MAX = 2
S3_VEL_GAIN_VN1 = 18
S3_VEL_GAIN_BASS = 15
S3_DUR_GAIN = 0.16
S3_EVENT_VEL_DROP = 10
S3_GRACE_RATIO = 0.20
S3_EVENT_THRESHOLD = 0.34
S3_MIN_EVENT_GAP_SEC = 0.33

# -----------------------------
# S5 ボレロ色付けパラメータ
# -----------------------------
S5_SNARE_BASE_VEL = 60
S5_SNARE_ENV_GAIN = 18
S5_SNARE_ACCENT_GAIN = 18
S5_SNARE_VELOCITY_GAIN = 14
S5_BD_UNDERLAY_DROP = 18
S5_HH_DROP = 32

S5_DENSITY_KEEP4_TH = 0.12
S5_DENSITY_KEEP3_TH = -0.18
S5_EVENT_THRESHOLD = 0.46
S5_EXTRA_HIT_DROP = 6

S5_YAW_STRONG_GAIN = 24
S5_YAW_PEAK_THRESHOLD = 0.48
...
# =========================================================
SECTIONS = [
    {"id": "S1", "name": "intro",      "bars": 52, "bpm": 62.40, "note_density": 0.190, "phrase_span": 0.300, "harmonic_tension": 0.174, "inner_voice_swing": 0.620},
    {"id": "S2", "name": "propulsion", "bars": 56, "bpm": 93.33, "note_density": 0.700, "phrase_span": 0.840, "harmonic_tension": 0.896, "inner_voice_swing": 0.220},
    {"id": "S3", "name": "peak1",      "bars": 36, "bpm": 83.88, "note_density": 0.640, "phrase_span": 0.740, "harmonic_tension": 0.560, "inner_voice_swing": 0.120},
    {"id": "S4", "name": "bridge",     "bars": 20, "bpm": 81.36, "note_density": 0.720, "phrase_span": 0.560, "harmonic_tension": 0.371, "inner_voice_swing": 0.090},
    {"id": "S5", "name": "peak2",      "bars": 48, "bpm": 74.81, "note_density": 0.770, "phrase_span": 0.540, "harmonic_tension": 0.315, "inner_voice_swing": 0.190},
    {"id": "S6", "name": "landing",    "bars": 8,  "bpm": 58.18, "note_density": 0.280, "phrase_span": 0.580, "harmonic_tension": 0.344, "inner_voice_swing": 0.780},
]
...
def melody_rhythm(note_density: float, bar_dur: float, is_landing: bool) -> list[tuple[float, float]]:
    if is_landing:
        return [(0.00 * bar_dur, 0.72 * bar_dur), (0.78 * bar_dur, 0.16 * bar_dur)]

    if note_density < 0.30:
        return [(0.00, 0.92 * bar_dur)]
    if note_density < 0.60:
        h = bar_dur / 2.0
        return [(0.0, 0.90 * h), (h, 0.92 * h)]
    if note_density < 0.78:
        q = bar_dur / 4.0
        return [(k * q, 0.92 * q) for k in range(4)]
    e = bar_dur / 8.0
...
def dyn_from_features(note_density: float, harmonic_tension: float,
    ...
    if section_name == "intro":
        ...
        vn1 = 36 + 36 * harmonic_tension + 10 * note_density
        vn2 = 25 + 20 * harmonic_tension
        va  = 23 + 18 * harmonic_tension + 8 * inner_voice_swing
        vc  = 28 + 18 * harmonic_tension + 10 * note_density
        ...
        for j, (off, dur) in enumerate(rhy_use):
            ...
            if section_id == "S1" and s1_ctrl is not None:
                ...
                if event > S1_EVENT_THRESHOLD:
                    ...
                    if (base_start - last_event_t) >= S1_MIN_EVENT_GAP_SEC:
                        ...
                        add_note(vn1, grace_pitch + VN1_OCTAVE_SHIFT, grace_start, grace_end, vel - S1_EVENT_VEL_DROP)
                ...
                note_dur = dur * s2_duration_scale(propulsion) * (1.0 + 0.04 * soft4 + 0.05 * soft + 0.08 * hard_cut)
                ...
                if event > S2_EVENT_THRESHOLD and hard_cut < 0.25:
                    ...
                    if (base_start - last_event_t) >= S2_MIN_EVENT_GAP_SEC:
                        ...
                        grace_end = grace_start + grace_dur
                        add_note(vn1, grace_pitch + VN1_OCTAVE_SHIFT, grace_start, grace_end, vel - S2_EVENT_VEL_DROP)
                ...
                if event > S3_EVENT_THRESHOLD:
                    ...
                    if (base_start - last_event_t) >= S3_MIN_EVENT_GAP_SEC:
                        ...
                        grace_end = grace_start + grace_dur
                        add_note(vn1, grace_pitch + VN1_OCTAVE_SHIFT, grace_start, grace_end, vel - S3_EVENT_VEL_DROP)
            ...
            if section_id == "S1" and s1_ctrl is not None:
                ...
                if note_dur < 0.28 or u < 0.30 or trans > 0.45:
                    prev = note
                    add_note(vn1, note + VN1_OCTAVE_SHIFT, base_start, base_start + note_dur, vel + 2 * trans)
                else:
                    ...
                    add_note(vn1, note + VN1_OCTAVE_SHIFT, base_start, split_t, vel)
                    add_note(vn1, tail_note + VN1_OCTAVE_SHIFT, split_t, base_start + note_dur, vel - 3)
                    prev = tail_note
            else:
                prev = note
                add_note(vn1, note + VN1_OCTAVE_SHIFT, base_start, base_start + note_dur, vel)
...
def add_s1_drum_parts(drum_inst, bar_start: float, bar_dur: float, s1_ctrl: dict):
    ...
    kick_vel = clamp(48 + 10 * env + 6 * acc, 22, 66)
    tom_vel  = clamp(kick_vel - 8, 16, 56)
    sub_vel  = clamp(kick_vel - 14, 12, 46)

    hh_layback = sixteenth * 0.08
    ...
    for k in range(8):
        ...
        if k % 2 == 0:
            ...
            add_note(drum_inst, DRUM_SOFT_TOM, st + hh_layback, st + hh_layback + eighth * 0.20, clamp(vel, 18, 70))

    if event > 0.50:
        extra_positions = [1.5, 5.5] if u_bar < 0.72 else [3.5, 6.5]
        for pos in extra_positions:
            st = bar_start + pos * eighth
            add_note(drum_inst, DRUM_SOFT_TOM, st + hh_layback, st + hh_layback + sixteenth * 0.42, clamp(sub_vel + 4, 14, 48))
    ...
    if u_bar >= 0.90:
        for k in [0, 2, 4, 6]:
            ...
            add_note(
                ...
                DRUM_SOFT_HH,
                st + hh_layback,
                st + hh_layback + sixteenth * 0.20,
                clamp(18 + 8 * ((u_bar - 0.90) / 0.10), 16, 28),
            )


# =========================================================
# S3 ドラム追加（第一メインを邪魔しない横ノリ）
# =========================================================
def add_s3_drum_parts(drum_inst, bar_start: float, bar_dur: float, s3_ctrl: dict):
    ...
    for beat in range(4):
        ...
        # 前に出す
        kick_vel = clamp(78 + 24 * arc_abs + 16 * acc + 8 * max(drive, 0.0), 52, 116)
        tom_vel  = clamp(kick_vel - 6 + 12 * max(drive, 0.0), 40, 102)
        hh_vel   = clamp(28 + 12 * max(drive, 0.0) + 8 * arc_abs, 18, 52)

        # 表のキック
        add_note(drum_inst, DRUM_SOFT_KICK, st, st + eighth * 0.56, kick_vel)

        # 2打目のトム
        add_note(
            drum_inst,
            DRUM_SOFT_TOM,
            st + eighth + hh_layback,
            st + eighth + hh_layback + eighth * 0.48,
            tom_vel,
        )
        ...
        add_note(
            drum_inst,
            DRUM_SOFT_HH,
            st + quarter * 0.50 + hh_layback,
            st + quarter * 0.50 + hh_layback + sixteenth * 0.24,
            hh_vel,
        )
        add_note(
            drum_inst,
            DRUM_SOFT_HH,
            st + quarter * 0.75 + hh_layback,
            st + quarter * 0.75 + hh_layback + sixteenth * 0.20,
            clamp(hh_vel - 4, 14, 46),
        )
        ...
        if event > 0.35:
            add_note(
                ...
                DRUM_SOFT_KICK,
                st + quarter * 0.72,
                st + quarter * 0.72 + eighth * 0.34,
                clamp(kick_vel - 4, 36, 100),
            )
            add_note(
                drum_inst,
                DRUM_SOFT_HH,
                st + quarter * 0.84 + hh_layback,
                st + quarter * 0.84 + hh_layback + sixteenth * 0.22,
                clamp(hh_vel + 6, 18, 56),
            )

        # フレーズ推進が強いときは、拍末に弱いトムを足して前進感を出す
        if drive > 0.25:
            ghost_st = st + quarter * 0.92
            add_note(
                drum_inst,
                DRUM_SOFT_TOM,
                ghost_st,
                ghost_st + sixteenth * 0.22,
                clamp(tom_vel * 0.55, 18, 58),
            )
...
def add_s5_drum_parts(drum_inst, bar_start: float, bar_dur: float, s5_ctrl: dict):
    ...
    for beat in range(4):
        ...
        for step in hit_steps:
            ...
            if step == 0:
                vel = clamp(base_vel + 14 + head_boost, 28, 124)
                dur = sixteenth * 1.60
            else:
                vel = clamp(base_vel - 5 + 5.0 * accent + 4.0 * env_abs, 20, 102)
                dur = sixteenth * 0.96

            if env_abs >= S5_YAW_PEAK_THRESHOLD and step == hit_steps[-1] and len(hit_steps) >= 3:
                vel = clamp(vel + 6, 20, 104)

            add_note(drum_inst, DRUM_SNARE_MAIN, st, st + dur, vel)

            # 横ノリ用ゴースト
            if step == 0 and accent >= 0.35:
                ghost_st = st + sixteenth * 0.65
                ghost_vel = clamp(vel * 0.38, 8, 42)
                add_note(
                    drum_inst,
                    DRUM_SNARE_MAIN,
                    ghost_st,
                    ghost_st + sixteenth * 0.30,
                    ghost_vel,
                )

        bd_vel = clamp(base_vel - S5_BD_UNDERLAY_DROP + 10 * env_abs, 8, 52)
        add_note(drum_inst, DRUM_BD_SUB, beat_start, beat_start + sixteenth * 0.92, bd_vel)

        # ハイハットを少し後ろへ
        hh_layback = sixteenth * 0.10
        hh_vel = clamp(base_vel - S5_HH_DROP + 6 * accent, 4, 26)
        add_note(
            drum_inst,
            DRUM_CLOSED_HH,
            beat_start + hh_layback,
            beat_start + hh_layback + sixteenth * 0.18,
            hh_vel,
        )

        if event >= S5_EVENT_THRESHOLD:
            extra_positions = [1.5, 2.5, 3.5] if env_abs >= 0.45 else [2.5]

            for pos in extra_positions:
                extra_st = beat_start + pos * sixteenth
                extra_vel = clamp(base_vel - S5_EXTRA_HIT_DROP + 10 * accent + 8 * env_abs, 16, 90)

                add_note(
                    drum_inst,
                    DRUM_CLOSED_HH,
                    extra_st,
                    extra_st + sixteenth * 0.26,
                    clamp(extra_vel - 12, 8, 52),
                )

            if env_abs >= S5_YAW_PEAK_THRESHOLD:
                extra_st = beat_start + 2.5 * sixteenth
                extra_vel = clamp(base_vel - S5_EXTRA_HIT_DROP + 12 * accent + 10 * env_abs, 20, 104)
                add_note(drum_inst, DRUM_BD_SUB, extra_st, extra_st + sixteenth * 0.76, extra_vel)
...
vn1   = pretty_midi.Instrument(program=pretty_midi.instrument_name_to_program("Cello"))
...
vc    = pretty_midi.Instrument(program=pretty_midi.instrument_name_to_program("Contrabass"))
...
for i, row in enumerate(PROGRESSION):
    ...
    if section_id == "S1":
        ...
        hold_ratio = clamp(hold_ratio * s2_duration_scale(propulsion), 0.84, 1.04)
        soft = s2_intro_softness(t)
        hold_ratio = clamp(hold_ratio * (1.0 + 0.06 * soft), 0.84, 1.08)

    elif section_id == "S3":
        u_bar = section_u("S3", t)
        drive = get_control_value(S3_CTRL, "phrase_drive", u_bar)
        hold_ratio = clamp(hold_ratio * s3_duration_scale(drive), 0.84, 1.06)
    ...
    if section_id == "S1":
        ...
        piano_chord_vel -= 3 * soft4 + 7 * soft + 8 * hard_cut
        piano_ext_vel  -= 2 * soft4 + 5 * soft + 7 * hard_cut
        piano_chord_vel += 2
    ...
    if len(pat) == 1:
        ...
        if section_id == "S2":
            ...
            vel += 2
            vel -= 4 * soft4 + 10 * soft + 12 * hard_cut
...
tail_start = t
tail_len = final_bar_dur * 2.1

for p in [69, 73, 76]:
    add_note(piano, p, tail_start, tail_start + tail_len, 36)

add_note(vn1, 76 + VN1_OCTAVE_SHIFT, tail_start, tail_start + tail_len * 1.02, 42)
add_note(vn2, 73, tail_start, tail_start + tail_len * 0.94, 30)
add_note(va,  69, tail_start, tail_start + tail_len * 0.92, 28)
add_note(vc,  45, tail_start, tail_start + tail_len * 1.06, 36)
